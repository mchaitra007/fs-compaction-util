...
def compact_files(spark_session: SparkSession, src_uri: str, dst_uri: str) -> None:
    ...
    if num_obj != 0:
        logger.info(f'\n*****  Compacting files for ={src_uri}, started with {num_obj:,d} files, {total_size:,d}.')
        try:
            num_partitions = math.ceil(total_size/(1000 * 1000 * file_size))
            start_time = time.time()
            df = spark_session.read.parquet(src_uri)
            logger.info(f'\n*******Number of original partitions = {df.rdd.getNumPartitions()}')
            logger.info(f'\n*******Read took {int(time.time() - start_time):,d} seconds')
            logger.info(f'\n****  Number of write Partitions ={num_partitions} *****')
            start_time = time.time()
            df = df.repartition(num_partitions)
            logger.info(f'\n**** Repartition took {int(time.time() - start_time):,d} seconds')
            start_time = time.time()
            df.write.mode('overwrite').parquet(dst_uri)
            logger.info(f'\n**** Write took {int(time.time() - start_time):,d} seconds')
